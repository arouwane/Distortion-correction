...
class PolynomialCameraNormalized:
    """
    ...
    def PGrid(self, size , alpha):
        
        roi = np.array([[0,0],[size[0],size[1]]])   # Plot (y,x) in inverted axis (image system)
        m,_= MeshFromROI(roi, dx=60)

        xn1, yn1  = m.n[:,0], m.n[:,1]
        px, py = self.P(m.n[:,0] , m.n[:,1]  )
        ...
        plt.gca().invert_yaxis()
        plt.axis('equal')
        m.Plot(n=np.c_[yn1,xn1]   , edgecolor='red', alpha=1, label='Reference')
        m.Plot(n=np.c_[yn1+(py-yn1)*alpha,xn1+(px-xn1)*alpha]  , edgecolor='blue', alpha=1, label = 'Deformed')
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
        plt.legend() 
